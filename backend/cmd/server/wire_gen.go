...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	balanceCardRepository := repository.NewBalanceCardRepository(db)
	balanceCardCache := repository.NewBalanceCardCache(redisClient)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, balanceCardRepository, balanceCardCache)
	...
	balanceCardService := service.NewBalanceCardService(balanceCardRepository, balanceCardCache)
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, balanceCardService)
	...
	openAIPAT401CounterCache := repository.NewOpenAIPAT401CounterCache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	httpUpstream := repository.NewHTTPUpstream(configConfig)
	leaderLockCache := repository.NewLeaderLockCache(redisClient)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, openAIPAT401CounterCache, settingService, compositeTokenCacheInvalidator, ollamaCloudUsageService)
	...
	balanceCardHandler := handler.NewBalanceCardHandler(balanceCardService)
	...
	adminBalanceCardHandler := admin.NewBalanceCardHandler(balanceCardService)
